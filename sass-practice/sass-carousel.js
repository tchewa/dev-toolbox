(function( $ ){
	...
	slideFunction = function() {
		...
		$( '.card-container' ).css( 'left', - dot * messWidth );
	}
	...
	$(window).on('load resize', function() {
		...
		if ($(window).width() > 650) {
			...
			$('.tile').on('mouseenter', function() {
				$(this).find('.hidden-tile-content').css('bottom', '0');
			});

			$('.tile').on('mouseleave', function() {
				$(this).find('.hidden-tile-content').css('bottom', '-55%');
			});
		} else {
			$('.hidden-tile-content').css('bottom', '0');
		}
	});
